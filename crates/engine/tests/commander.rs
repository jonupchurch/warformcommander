...
use engine::model::types::{MachineTypeId, VariantId, ZoneId};
...
fn command_aura_raises_army_damage() {
    let rs = anvil_rs();
    // Four Grizzly attackers + one Rear support (the variable: Commander vs plain support).
    let army = |support: &str| Army {
        machines: vec![
            ...
            stock_instance(&rs, MachineTypeId::RearSupport, support, ZoneId::Rear, 4),
        ],
    };
    let with_commander = damage_dealt_a(&run(&rs, army("CommandPost"), anvils(&rs), 0xC0DE));
    let with_warden = damage_dealt_a(&run(&rs, army("Warden"), anvils(&rs), 0xC0DE));
    assert!(
